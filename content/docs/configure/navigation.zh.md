...
### 自动隐藏与唤醒区域 {#navbar-autohide}

设置 `params.ui.navbar_autohide: true`，或在分区 cascade 中设置
`navbar_autohide: true`，可把顶栏收在视口上方。在 768px 及以上、使用精细指针（鼠标或触控板）时，把指针移入顶栏原位置上方 60% 的中间区域即可唤醒顶栏。最左和最右各 64px 不属于唤醒区，折叠后的侧栏与大纲恢复按钮因而仍可点击；两个按钮同时与 breadcrumb、RSS/复制工具行保持同一水平位置。

小于 768px 或使用粗指针/触屏时，自动隐藏会停用，普通 sticky 顶栏始终可见。显式按下
<kbd>H</kbd> 进入无界面阅读模式时，仍会按该快捷键的定义隐藏全部chrome。
...
当顶层菜单直接指向 `/tags`
之类的 taxonomy 页面时，不需要手工配置子菜单。OINK 会自动按数量降序渲染
`标签 + 数量` chip：
...
切换器是全局的：它包含所有顶层内容分区，以及站内所有设置了
`sidebar_root_for: self` 的分区。若某个顶层工具分区不应出现，可在它的 front
matter 中设置
`sidebar_root_menu: false`。只有一个入口时，控件退化为无边框普通链接；两个及以上入口才显示下拉菜单。

切换器下方的导航树仍把栏目落地页本身作为第一个链接。这是有意区分：切换器选择一棵树，根链接选择一篇文档。因此根页可被 W/S 聚焦，也能进入 Q/E 和上一页/下一页序列。

分类术语页没有内容层级，因此术语会采用其成员共同所属的顶层分区。从文档页面点进某个标签后，侧边栏保留的仍是文档树和文档根链接，而不会回退到站点级的树；成员横跨多个分区的术语则不显示根节点行。

## 页面目录 {#table-of-contents}

Hugo 根据 Markdown 标题生成右侧页面大纲。OINK 把它渲染为固定右栏中的第一个分组，后面依次排列当前分区的分类标签云。首行和正文的面包屑/操作上下文行共享同一个顶部间距 token，因此两侧正好对齐。读者可以折叠整个右栏，状态保存在本地。
...
普通内容页上方和分类结果中会显示面包屑，这一行同时承载页面操作。顶层分区或分类索引会省略重复的一级面包屑；只要页面操作存在，上下文行仍保留完全相同的几何尺寸。全局关闭面包屑的方式如下：
...
页面操作是页面上下文行末尾的一个纯图标拆分按钮。左半边复制本页 Markdown，成功后翻转为绿色对勾；右侧箭头展开一个包含十项操作的菜单，分为两组。上半组负责把页面内容带到别处：
