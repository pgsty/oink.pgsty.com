...
| 模式     | 触发                              | 内容                         |
| -------- | --------------------------------- | ---------------------------- |
| 空查询   | `Cmd/Ctrl-K`、`/` 或 `f` 后不输入 | 快捷入口、页面操作、偏好设置 |
| 文本查询 | 直接输入                          | 按分区分组的页面结果         |
| 命令模式 | `\`、`c` 或输入 `>` 前缀          | 只搜索命令，不查页面         |

`/` 打开完整搜索，`\`
只打开命令。两者都是单字符快捷键，所以只在可编辑控件之外生效——在 input、textarea、select 和 contenteditable 区域里输入这些字符仍然是输入文本。带修饰键的组合（例如
`Ctrl-/`）也不会触发。

面板已打开时按 `/` 或 `\` 不会清空当前查询。键盘导航提供的别名 `f` 与 `c`
对应相同的两种模式，详见[键盘导航](../keyboard/)。
...
- **偏好设置**：切换版本、语言或配色
- **命令**：打开项目仓库，随后列出站点自定义命令

浏览状态下的列表与导航栏控件顺序一致：版本、语言、主题、GitHub，站点自定义命令排在内建项之后；输入查询后仍按相关度排序。
