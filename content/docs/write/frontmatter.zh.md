...
顺序固定为反馈 → 出处 → 反向链接 → 翻页器 → 评论，见[编写页面](/zh/docs/write/pages/#page-end)。
...
| `backlinks` | 布尔 | 站点 `params.ui.backlinks`（关） | 页尾出处与翻页器之间是否列出链接到本页的页面，分区可以 cascade。只接受布尔，其它写法告警并回退，见[导航与菜单](/zh/docs/customize/navigation/#backlinks) |
