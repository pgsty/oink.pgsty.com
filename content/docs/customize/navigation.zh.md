...
右栏可以列出有哪些页面链接到这一页：一个带链接图标的「反向链接」组，排在目录下方、分类标签云上方，默认展开；低于 `xl` 断点时，它随目录一起进入侧栏抽屉。从搜索落到这一页的读者由此看到哪些页面认为它值得指向，也看到它在站点其余部分里的位置。默认关闭，由站点打开：
...
页面的 Markdown 输出带同一份列表，前缀是「反向链接：」。RSS 省略它，`print` 输出格式连同整个右栏一起省略。

本站全站开启了它：看本页右栏的「反向链接」组就是实际效果；被引用最多的[配置总览](/zh/docs/customize/config/)一页，列出了四十多个入链，其中大部分收在「再显示 N 条」里。
