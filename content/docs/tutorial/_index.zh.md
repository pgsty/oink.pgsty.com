---
title: 快速上手
description: 使用 Hugo Extended 构建双语 Oink 文档站。
date: 2018-07-30
...
- **Hugo Module（推荐）**：在已有或新建 Hugo 站点中导入
  `github.com/pgsty/oink`。参阅 [Oink 快速开始](/zh/docs/tutorial/install/)。
- **项目站点**：把独立的
  [`pgsty/oink.pgsty.com`](https://github.com/pgsty/oink.pgsty.com)
  仓库作为完整双语配置与回归参考。
- **现有 Docsy 站点**：按照[迁移指南](/zh/docs/upgrade/migrate-from-docsy/)删除公共覆盖和消费端 npm 资源管线，无需重写正文。

## 安装前提条件 {#install-the-prerequisites}

安装 Git、Go 与 Hugo Extended `{{% param hugoMinVersion %}}`
或更高版本。平台说明和验证命令请参阅[开始之前](prerequisites/)。
