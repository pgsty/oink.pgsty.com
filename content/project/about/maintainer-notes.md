...
> [!IMPORTANT]
>
> This page preserves detailed upstream Docsy monorepo procedures for historical
> reference. Oink now uses separate theme and project-site repositories. Follow
> [Git repository layout and branch model](/project/build/git-repo/) and the
> current [Oink release process](/docs/oink/release/) for repository paths,
> module tags, validation, and publication.
...
- [oink.pgsty.com/config/_default/hugo.yaml][] `params.hugoMinVersion`, which
  feeds the requirement statements in user-facing docs (via
  `{{%/* param hugoMinVersion */%}}`) and, through the `&hugoMinVersion` anchor,
  oink.pgsty.com's own `module.hugoVersion.min`.
...
`hugo-extended` dev dependency in [oink.pgsty.com/package.json][].
...
(`hugo.Version`): oink.pgsty.com builds always run the pinned Hugo.
...
`test:website` checks oink.pgsty.com's links with Lychee, caching external-link
results in the committed `oink.pgsty.com/.lycheecache` (the "refcache") so
checks stay fast and offline-friendly. Config lives in
`oink.pgsty.com/lychee.toml`. CI installs a pinned lychee binary (see
...
2. **Retire temporary measures** that the shipped release makes obsolete,
   ...
   - Remove any temporary ignore rules from `oink.pgsty.com/lychee.toml` and
     confirm that the link check passes.
...
7. **A/B diff the generated site**:
   - If the site's `public/` folder is a git repository (a setup worth adopting;
     see oink.pgsty.com's `make:public` npm script), build at the current
...
[oink.pgsty.com/config]: <{{% param github_repo %}}/blob/main/oink.pgsty.com/config/>
[oink.pgsty.com/config/_default/hugo.yaml]: <{{% param github_repo %}}/blob/main/oink.pgsty.com/config/_default/hugo.yaml>
[oink.pgsty.com/package.json]: <{{% param github_repo %}}/blob/main/oink.pgsty.com/package.json>
