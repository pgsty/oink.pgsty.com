...
完整参数契约见[代码块](/zh/docs/content/code-blocks/)。
...
各组件契约见[组件](/zh/docs/content/components/)。
...
页面操作与面板命令现在走同一套注册表，因此复制 Markdown、打印、切换主题、语言或版本，无论从哪里触发行为都一致。
...
本站一个典型文档页的 CSS 与 JavaScript 体积，从 755,512 字节降到 667,729 字节。
...
2. 把原始 `body` 或标题字体覆盖迁移到字体角色。
3. 更新 Hugo 模块并整理模块图。
4. 构建站点，检查有代表性的文档页、博客页、移动端、打印视图与明暗模式。
...
0.3.0 候选版本通过并列的 Oink 项目站进行验证，因此站点构建针对的是候选主题本身，而不只是它最后固定的发布版本。主题侧的门禁覆盖十五项契约检查、在最低与当前 Hugo 版本上零警告构建示例站、两种字体预设，以及浏览器运行时单元测试。站点侧的门禁覆盖格式化、中英文页面配对与稳定标题 ID、渲染后的 Markdown 与站内链接、Hugo 模块 fixture、备用配置构建、Markdown 与 favicon
golden、响应式与组件浏览器行为，以及 axe 无障碍检查。
