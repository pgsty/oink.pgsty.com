...
description: >-
  ...
  全文包，以及以 JSON 发布的导航树；再加上静态反向链接，让页面末尾列出链接到它的页面。
  三者都默认关闭，不点名就不会出现。
authors: [oink]
categories: [release]
tags: [Oink, Release]
...
第三项是给作为人的读者的：页面末尾现在可以列出有哪些页面链接到它。
...
- `params.ui.backlinks` 让页面末尾列出链接到它的页面，索引在构建时从你 Markdown 里本来就有的链接派生。
...
反向链接补上的就是这另一半：页面末尾列出有哪些页面链接到它，位置在页面信息与翻页器之间，
标题是「反向链接」。一个键就能打开：
...
细节见[反向链接](/zh/docs/customize/navigation/#backlinks)。
