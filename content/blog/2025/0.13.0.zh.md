...
- {{% _param FAS compass "" %}}
  [导航与用户体验](#navigation-and-ux)改进，包括 2025 年**得票最高**的[功能请求][request of 2025]：[目录活动项跟踪](#active-toc-entry-tracking)（[#349][]、[#2289][]）；
- {{% _param FAS code "" %}}
...
Docsy
0.13.0 引入_目录（TOC）活动项跟踪_，这是 2025 年**得票最高**的[功能请求][request of 2025]。当读者滚动页面时，当前可见分区对应的目录项会高亮。该功能通过 Bootstrap
[ScrollSpy][]
...
- <i class="fa-solid fa-palette fa-lg text-info px-1"></i>
  主题整体的**颜色对比度**得到改善，Docsy 现在会回退到 Bootstrap 的字体与颜色默认值，从而提供更好的开箱即用无障碍合规性。详情见
  [#2285][] 和[站点颜色][]。
...
- {{% _param FAS moon info %}} **[深色模式][]**：
  ...
  - {{% _param FAS search info %}}
    **Google 搜索**结果页支持深色模式（[#2387][]）。{{%_param BADGE EXPERIMENTAL info %}}
