...
## Designing safe ECharts

ECharts exposed the sharpest migration edge. Existing pages sometimes placed
arbitrary JavaScript functions in chart blocks. Simply preserving that behavior
would make executable content the default API; banning it immediately would
break proven production pages.

The solution separates modes:

1. new content supplies JSON or YAML, which Hugo parses and safely serializes;
2. JavaScript is rejected by default;
3. a reviewed legacy instance can set `unsafe=true`;
4. a site-wide `params.content.echarts_unsafe` switch exists only as a migration
   bridge.

Tests include a `</script>` boundary payload, repeated identical charts, invalid
CSS lengths, and explicit acceptance and rejection cases.
...
24 from Pigsty. Pigsty retained its three business matrix shortcodes and used
the explicit legacy ECharts bridge. All temporary copies built, producing 1,095,
16, 128, and 2,473 HTML files respectively.
...
- unsafe ECharts rejection made the security boundary executable;
