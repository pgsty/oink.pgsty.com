...
- {{% _param FAS universal-access "" %}}
  改进[无障碍能力](#accessibility)，增强颜色对比度和深色模式支持。

## 准备升级？<a id="breaking-changes"></a> {#ready-to-upgrade}

- 审阅 {{% _param BADGE BREAKING warning %}} 变更：
  - {{% _param BREAKING %}} [告警短代码](#alert-shortcode)正文处理方式变化；
  - {{% _param BREAKING %}} [语言菜单](#language-menu-visibility)可见性变化。
- 可以快速浏览：
  - {{% _param NEW %}} 新功能；
...
## {{% _param FAS compass "" %}} 导航与用户体验改进 {#navigation-and-ux}

### {{% _param NEW %}} 目录活动项跟踪 {#active-toc-entry-tracking}
...
### {{% _param NEW %}} 分区侧边栏根节点 {#section-sidebar-root}
...
### {{% _param BREAKING %}} 语言菜单可见性 {#language-menu-visibility}
...
这是一项 {{% _param BADGE BREAKING warning %}}
...
## {{% _param FAS universal-access info %}} 无障碍改进 {#accessibility}
...
- {{% _param FAS palette info %}} 深色模式的 **颜色对比度** 改进：
  - 修复用户偏好与系统设置不同时的目录项颜色对比度（[#2379][]）；
  - 为使用 Bootstrap 主题变量的项目提供早期实验性支持，允许定制对比度调整（[#2384][]）。详情见[选择具有良好对比度的颜色][]。{{% _param BADGE EXPERIMENTAL info %}}

- {{% _param FAS moon info %}} **[深色模式][]**：
  - {{% _param FAS bolt warning %}}
    修复[无样式内容闪烁][]（FOUC）（[#2332][]）；
  - {{% _param FAS search info %}} **Google 搜索**
    结果页支持深色模式（[#2387][]）。{{%_param BADGE EXPERIMENTAL info %}}
...
## {{% _param FAS rocket primary %}} 升级到 0.13.0 {#upgrade}

### 前提条件 {#prerequisites}

> {{% _param FAS triangle-exclamation warning %}} **我们建议** 先阅读 [Docsy
> 0.12.0 升级指南][升级到 Docsy 0.12.0]，因为该版本包含重要破坏性变更。

### 升级流程与 AI 辅助 {#upgrading-and-ai}

> {{% _param FAS robot info %}} 你是否尝试过用 AI 协助升级 Docsy？它能帮上大忙！
