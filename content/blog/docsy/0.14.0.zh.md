...
> - <i class="fa-solid fa-book text-info fa-lg"></i>
>   <span>**内容与本地化**：新增 Markdown
...
- 审阅 {{< badge text="BREAKING" tone="warning" >}} 变更：
  ...
  - <i class="fa-solid fa-triangle-exclamation fa-lg text-warning px-1"></i>
    [标题别名与页内目标](#heading-aliases)；
  - <i class="fa-solid fa-triangle-exclamation fa-lg text-warning px-1"></i>
    [blocks/cover 短代码](#blocks-cover)正文处理；
  - <i class="fa-solid fa-triangle-exclamation fa-lg text-warning px-1"></i>
    [Hugo 0.155.0 要求与 0.153+ 破坏性变更](#hugo)；
...
导航栏 **高度与样式**
可通过以下变量调整（{{< badge text="EXPERIMENTAL" tone="info" >}}）：
...
- <!-- markdownlint-disable no-space-in-emphasis -->
  ...
  **导航栏浅色/深色主题**：始终使用深色导航栏是 Docsy 早期限制，并不适合所有站点。现在可以选择最符合项目整体设计的主题。
  ...
  - <i class="fa-solid fa-triangle-exclamation fa-lg text-warning px-1"></i>
    如果设计确实要求深色导航栏，把 `params.ui.navbar_theme` 设为
...
- **导航栏 Partial 覆盖项**：适用于覆盖导航栏 Partial 的项目。请审阅
  ...
  > [!DETAILS]- `layouts/_partials/navbar.html` 编辑摘要
  >
...
- <i class="fa-solid fa-triangle-exclamation fa-lg text-warning px-1"></i>
  **适用条件**：站点使用 `td-offset-anchor`，例如覆盖
...
> [!DETAILS]- 移入 `td/` 子目录的内部 `assets/scss/` 文件列表
>
> ```text
> assets/scss/
> ├── _alerts.scss
...
- <i class="fa-regular fa-square-check fa-lg text-success px-1"></i>
  **建议多数项目采用**。[^below-navbar-default] 如果希望 `blocks/cover`
  位于导航栏下方而不是其后，请在调用中加入 `td-below-navbar` 辅助 class，例如
  `height="auto td-below-navbar"`。详见[导航栏下方高度调整][]。
...
- <i class="fa-solid fa-triangle-exclamation fa-lg text-warning px-1"></i>
  **适用条件**：使用 `td/code-dark`，而且希望恢复 `tango`/`onedark`
  样式。请按照[浅色/深色代码样式及其他配置][]操作；
...
### <i class="fa-solid fa-globe px-1"></i> 国际化 {#internationalization}
